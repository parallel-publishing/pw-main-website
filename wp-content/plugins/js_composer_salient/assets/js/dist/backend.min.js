...
window.vc || (window.vc = {}),
    ...
    function($) {
        ...
        _.isUndefined(window.vc) && (window.vc = {}), vc.showSpinner = function() {
          ...
          /* nectar addition to add wp 5.6 compatibility - .removeProp to .prop */
          var _this = $(this),
        ...
        vc.edit_form_callbacks = [], vc.atts = {
            ...
            init: function(param, $field) {
                $(".vc_link-build", $field).on("click", function(value_object) {
                    ...
                    jQuery(document).off("wplink-open", onOpen).on("wplink-open", onOpen), jQuery(document).off("wplink-close", onClose).on("wplink-close", onClose), "admin_frontend_editor" === vc_mode ? dialog.open("vc-hidden-editor") : dialog.open("content"), _.isString(value_object.url) && ($("#wp-link-url").length ? $("#wp-link-url") : $("#url-field")).val(value_object.url), _.isString(value_object.title) && ($("#wp-link-text").length ? $("#wp-link-text") : $("#link-title-field")).val(value_object.title), ($("#wp-link-target").length ? $("#wp-link-target") : $("#link-target-checkbox")).prop("checked", !_.isEmpty(value_object.target)), $("#vc-link-nofollow").length && $("#vc-link-nofollow").prop("checked", !_.isEmpty(value_object.rel)), $vc_link_submit.unbind("click.vcLink").bind("click.vcLink", function(string) {
                        var options;
    ...
    function($) {
        ...
        window.VcRowView = vc.shortcode_view.extend({
            ...
            addTab: function(tab_id) {
                // nectar addition vc_tab to tab
                if (tab_id && tab_id.preventDefault && tab_id.preventDefault(), !vc_user_access().shortcodeAll("tab")) return !1;
            ...
            cloneModel: function(model, parent_id, tag) {
              ...
              new_order = _.isBoolean(tag) && tag === true ? model.get('order') : parseFloat(model.get('order')) + vc.clone_index,
              model_clone,
